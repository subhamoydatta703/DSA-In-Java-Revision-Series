...
This repository is a Java practice workspace for Data Structures and Algorithms. It currently contains 164 Java source files grouped by topic. Most files are standalone `main` classes that can be compiled and run from the repository root.
...
| `binarysearch/` | 8 | Binary search examples for target lookup, insert position, finding first/last occurrences of a target, rotated sorted array searches, as well as finding lower/upper bounds (largest element less than or equal to target, and minimum element greater than or equal to target). |
...
- Binary search for direct target lookup, insert position, finding first, last, or both occurrences of a target, rotated sorted array search, and finding bounds (largest less/equal and minimum greater/equal elements)
