...
This repository is a Java practice workspace for Data Structures and Algorithms. It currently contains 127 small, focused programs grouped by topic. Most files are standalone `main` classes that can be compiled and run from the repository root.
...
- Variable-size sliding-window problems for longest bounded-sum subarrays, smallest subarrays meeting a target sum, longest subarrays with at most `k` distinct elements, max consecutive ones with flips, and substrings without repeating characters
