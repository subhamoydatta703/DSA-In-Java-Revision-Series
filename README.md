...
This repository is a Java practice workspace for Data Structures and Algorithms. It currently contains 212 Java source files grouped by topic. Most files are standalone `main` classes that can be compiled and run from the repository root.
...
| `stack/` | 3 | Stack operations implemented recursively (pushing to the bottom, reversing a stack) and queue implementation. |
...
- Stack operations using recursion (e.g., reversing a stack, pushing an element to the bottom) and queue implementation using a stack
